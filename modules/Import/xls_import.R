...
xls_import <- function(
  ...
  output$file_input <- shiny::renderUI({
    ...
    shiny::fileInput(
      inputId = ns("file"),
      label = "Upload an xls or xlsx file"
    )
  })
  ...
  file_type_supported_r <- shiny::reactive({
    file_ending_r() %in% c("xls", "xlsx")
  })
  
  output$file_type_not_supported <- shiny::renderUI({
    if (!file_type_supported_r()) {
      paste0("File type .", file_ending_r(), " not supported. Please upload an xls or xlsx file.")
    } else {
      NULL
    }
  })
  ...
  output$finish <- shiny::renderUI({
    ...
    shiny::validate(
      ...
      shiny::need(
        is.data.frame(data_r()), "Uploaded file must contain a data frame."
      )
    )
